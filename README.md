...
The Brave browser requires a one-time action before installation. Without the symbolic link, the `swupd` command will fail to add `brave-browser-stable` to the `/opt/3rd-party/bin` location.
